...
$text = "<label>Trip List Reference :</label><ul>";
...
while( $row_1 = mysql_fetch_array( $items_1,MYSQLI_ASSOC ) ) {
	...
	$text .= "<tr>";
	$text .= "<td width='70%'>";
	$text .= "<span style='float: left;'>$row_1[firstname] $row_1[lastname]:<br > $row_1[back_to_city], $row_1[back_to_country] to $row_1[travel_to_city], $row_1[travel_to_country],<br> $backdate</span>";
	$text .= "</td>";
	$text .= "<td width='30%'>";
	$text .= "<button class='btn btn-primary ladda-button' type='button' onclick='setidtrip($row_1[tripID])' />Titipin</button>";
	$text .= "</td>";
	$text .= "</tr>";
	$text .= "</tbody>";
	$text .= "</table>";
...
if ($text=='<label>Trip List Reference :</label><ul></ul>') {
